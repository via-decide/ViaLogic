# Archimedes — references

- **Validation ID:** VIA-HIST-ARCHIMEDES-REFERENCES
- **Version:** 1.1
...
## Engineering summary
References are the audit trail for claims about Archimedes. This page separates local repository evidence from authoritative source categories needed for future expansion.

## Historical summary
The local metadata records the following contribution marker: hydrostatics, levers, approximation methods.. Dates are recorded as birth `c. 287 BCE` and death `212 BCE` when available in local metadata.

## Repository references
- `data/entities.json`
- profile path `people/archimedes/` where available
- this history package under `docs/history/archimedes/`
...
- **Contribution marker:** local history package and local metadata: hydrostatics, levers, approximation methods..
...
## Modern relevance
A historical engineering platform is only useful if claims can be audited. This page defines the evidence boundary for Archimedes.
