...
```mermaid
graph LR
  A[Earlier practices and inherited problems] --> B[Euclid]
  B --> C[geometry and axiomatic method]
  C --> D[CAD, graphics, robotics geometry, formal proof, and engineering drawing]
```

## Influenced by
Euclid should be read against earlier tools, teachers, texts, institutions, and technical needs. Influence is strongest when a source shows direct reading, correspondence, apprenticeship, translation, patronage, or explicit criticism.
