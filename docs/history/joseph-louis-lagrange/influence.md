...
```mermaid
graph LR
  A[Earlier practices and inherited problems] --> B[Joseph Louis Lagrange]
  B --> C[science]
  C --> D[later education, scientific vocabulary, social organization, technical reasoning, and interpretive frameworks]
```

## Influenced by
Joseph Louis Lagrange should be read against earlier tools, teachers, texts, institutions, and technical needs. Influence is strongest when a source shows direct reading, correspondence, apprenticeship, translation, patronage, or explicit criticism.
