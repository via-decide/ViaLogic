...
- **Validation ID:** VIA-HIST-VOLTAIRE-REFERENCES
- **Version:** 1.1
- **Last reviewed:** 2026-07-09
- **Repository source:** `data/entities.json`; profile path `people/voltaire/` where available
...
## Engineering summary
References are the audit trail for claims about Voltaire. This page separates local repository evidence from authoritative source categories needed for future expansion.

## Historical summary
The local metadata records the following contribution marker: Voltaire · Classical.. Dates are recorded as birth `not specified in local metadata` and death `not specified in local metadata` when available in local metadata.

## Repository references
- `data/entities.json`
- profile path `people/voltaire/` where available
- this history package under `docs/history/voltaire/`
...
- **Contribution marker:** local history package and local metadata: Voltaire · Classical..
...
- **Repository links:** `data/entities.json`; profile path `people/voltaire/` where available.

## Modern relevance
A historical engineering platform is only useful if claims can be audited. This page defines the evidence boundary for Voltaire.
