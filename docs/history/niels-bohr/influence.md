...
```mermaid
graph LR
  A[Earlier practices and inherited problems] --> B[Niels Bohr]
  B --> C[science]
  C --> D[later education, scientific vocabulary, social organization, technical reasoning, and interpretive frameworks]
```

## Influenced by
Niels Bohr should be read against earlier tools, teachers, texts, institutions, and technical needs. Influence is strongest when a source shows direct reading, correspondence, apprenticeship, translation, patronage, or explicit criticism.
